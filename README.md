...
- 自主图片和自动头像提示会结合本地时段、稳定外观、已知服装、场景和最近事件，不会凭时间臆造天气。
- 角色自主发图支持每日预算、会话冷却、提示词去重和用户边界；生图失败不影响已显示文字，可在原事件上重试。
- 图片理解以结构化观察、置信度和不确定项进入对话；低置信度 OCR、人脸身份和敏感属性不作肯定陈述。
...
- 主动消息支持全局及每角色关系节奏、称呼与话题边界、频率、每日上限、静默、静音和临时暂停；
  未通过本地门禁不会调用模型，角色页可以查看最近一次允许或拦截原因。
...
docs/ROLEPLAY_OPTIMIZATION_TODO.md 角色扮演优化完成项与后续路线
docs/ROLEPLAY_BASELINE.md      角色扮演离线质量基线与供应商冒烟矩阵
docs/DIAGNOSTICS.md            本地诊断、隐私边界与脱敏导出说明
docs/TEXT_MODEL_ROUTING.md     主角色/辅助模型职责、能力协商与降级
docs/MEMORY_GOVERNANCE.md      角色记忆来源、纠正、删除、迁移与同步规则
docs/DIRECTOR_ACTOR.md         关键轮次 Director→Actor 契约、触发与回退
docs/RELATIONSHIP_POLICY.md    关系边界、主动消息门禁、角色覆盖与双端幂等
docs/MULTIMODAL_PIPELINE.md     文本、图片与 TTS 唯一事件源、状态与失败隔离
...
- 全面升级 BanVerse 浅色与深色主题，采用更清晰的翡翠绿品牌色和分层中性色。
- 统一按钮、输入框、下拉框、标签页、设置卡片、列表选中态和滚动条的视觉规范。
- 聊天标题栏增加角色头像与会话副标题，优化模型选择器宽度和信息层级。
- 区分角色对白、用户消息、旁白、输入状态及错误消息气泡，提升聊天可读性。
- 优化角色库、会话列表、附件预览和消息输入区，并保持安卓端 44px 最小触控区域。
- 提升深色主题强调文字和控件焦点态的对比度，不改变现有对话、同步、图片或 TTS 逻辑。

## 1.6.0 更新

- 新增本地脱敏诊断和角色质量基线，可区分主回复、摘要、图片、TTS 与同步失败，且默认不上传遥测。
- 文本设置拆分主角色模型与辅助任务模型，按平台和实际模型能力安全发送采样参数。
- 新增可选 Director → Actor 关键轮次规划；非法结果、超时和取消均降级为普通单次回复，隐藏规划不进入消息、TTS 或同步。
- 引入可追溯的角色记忆治理，支持查看、纠正、固定、删除、保留期限与每角色开关，不把模型推测当成已确认事实。
- 新增全局及每角色的关系节奏、称呼、话题边界、主动消息频率、静默、暂停与双端幂等租约。
- 统一文本、图片和 TTS 的分段事件顺序；图片增加稳定视觉身份、结构化识图、预算/冷却/去重/边界和原事件重试。
- TTS 按气泡队列只朗读角色台词，排除旁白、括号动作、图片指令和停顿/拼音/语速/音量控制标记。
