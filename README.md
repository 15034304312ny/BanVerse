...
- 设置页“新角色发现”可独立启用随机新联系人。软件运行期间到达随机时间后，当前文本平台会生成一张受控的原创成年 Character Card V2；通过格式、重名、目标性别和高优先级指令过滤后写入本机，并用角色的第一条消息创建联系人会话。可配置 15 分钟至 7 天的随机区间、每日 1 至 10 位的成功上限及女性/男性生成比例，默认关闭、默认每日最多 1 位、默认性别比例各 50%。失败或重名不占成功名额，自动角色可照常编辑和删除。
- 角色自主发图、用户索图和自动角色头像的绘图提示会读取设备当前本地时段，分别使用清晨、上午、午间、下午、傍晚、晚间、深夜或凌晨对应的光线和生活场景；不会仅凭时间臆造天气，非现代世界观会换算为相同时段的合理环境。
