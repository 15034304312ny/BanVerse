...
- 从角色开始聊天时，首次使用 `first_mes`，之后按已有会话数轮换 `alternate_greetings`；开场白会显示并保存，不是一次模型请求。
...
桌面 App 在每条成功回复后会把该回复再次发送到 DeepSeek Flash 生成消息列表摘要。角色会话在同一次后台请求中同时更新结构化连续性状态，因此不会为记忆维护再增加一次请求。对白、旁白和发图事件的分类与拆分完全在本机完成；发图动作和用户明确索图会形成可靠的本地兜底条件，同时 DeepSeek Flash 独立进行一次语义发图判断，两者按 OR 逻辑合并并在单轮内去重。生图失败会保留已经投递的文字消息。角色提示会注入设备本地时间快照，以便普通回复和主动消息正确理解当前日期、星期及生活时段；启用“角色主动消息”后，最近上下文、连续性状态和时间提示会在随机计时到期时发送到 DeepSeek，以生成一条时机合适且不重复近期话题的角色消息。主动消息默认关闭。
