...
OPENING_SYSTEM_SUFFIX = """## 首次开场
这是角色与用户在这段新会话中的第一条消息。请根据角色设定和当前本地时间自然地打招呼、
分享一个符合角色生活的当下细节，或提出一个容易回应的话题。不要假装双方刚刚聊过，
...
def opening_request(
    ...
    *,
    ...
    """生成新角色会话的首次联系请求，不携带模板或伪造历史。"""

    context = local_time_context(current_time)
    return (
        f"请以{character_name}的身份，在一段全新的会话中主动给用户发第一条消息。\n"
        f"{context.prompt_text}\n"
        "这是首次开场，没有可延续的聊天历史；直接输出角色会发送的内容。"
    )
